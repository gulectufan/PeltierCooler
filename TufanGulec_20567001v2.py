...
# bilgi alanı
        self.labelPort = QtWidgets.QLabel('<font color=red>Please select a serial port of a device to connect </font>')
...
# message
        ...
        self.messageTitle = QtWidgets.QLabel("Logger Monitor")
        ...
        if self.baglan.text() == "Connect":
            if (self.mySerial.serialPort.isOpen() == False):
                ...
                try:
                    ...
                    self.message.append("Connection error!")  #log
                if (self.mySerial.serialPort.isOpen()):
                    self.labelPort.setText('<font color=green> Connected to Arduino Promini 3.3V </font>')  #window
                    self.message.append("Connectted") # log
            ...
            if self.mySerial.serialPort.isOpen():                
                ...
                if self.sicaklik2.text() == "Stop reading for fan":
                    ...
                    self.sicaklik2.setStyleSheet("font: bold 14px; background-color : red; max-width: 15em; padding: 6px")
                    self.labelSicaklik2.setText('<font color=red> Fan temperature measuring was stopped </font>')
                    #self.message.append("GUI : ")
                self.sicaklik2.setEnabled(False)
                """           
                self.mySerial.serialPort.close()
                if self.mySerial.serialPort.isOpen() == False:
                    self.labelPort.setText('<font color=red> Disconnected </font>')
                    self.message.append("Disconnectted ")
...
# 
    def messageTextEdit(self):  # seri porttan mesaj geldiginde bu fonksiyon calisacak
        ...
        if (str(self.veri[0]) == "tank") and (str(self.veri[1]) != "okunamadi"):
            ...
            if len(self.dataTank) < 5:
                self.dataTank = self.dataTank + [self.TankC]
                self.dataFan = self.dataFan + [self.CoolerC]
            else:    
                if not(self.CoolerC < (self.dataTank[len(self.dataTank)-1]-0.3)) or not(self.CoolerC > (self.dataTank[len(self.dataTank)-1]+0.3)):
                    self.dataTank = self.dataTank + [self.TankC]
                    self.dataFan = self.dataFan + [self.CoolerC]
    ...
    def funcs(self,func):  
        ...
        if func == self.pump:
            if self.pump.text() == "Pump Off":
                if self.mySerial.serialPort.isOpen():
                    ...
                    self.labelPump.setText('<font color=green> Pump turned on </font>')
                else:
        ...
        if func == self.sicaklik:
            if self.sicaklik.text() == "Tank Temperature":
                ...
                self.labelSicaklik.setText('<font color=green> Tank temperature is measuring </font>')
